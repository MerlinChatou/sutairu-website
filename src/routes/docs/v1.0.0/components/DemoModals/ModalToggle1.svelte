<div id="toggle-modal-1" tabindex="-1" class="modal" data-close-modal>
    <div class="modal-content">
        <div class="header">
            <h4 class="mt-0">Modal 1</h4>
            <button type="button" class="btn-close" data-close-modal aria-label="Close"></button>
        </div>
        <div class="body">
            <p class="mt-0">Click the button to open the second modal.</p>
            <button class="btn bg-green btn-lg w-full" data-open-modal="toggle-modal-2">Open second modal</button>
        </div>
      <div class="footer text-right">
        <button class="btn bg-accent" data-open-modal="documentation-modal" data-documentation="extras/modal">Documentation</button>
        <button class="btn bg-gray" data-close-modal>Close</button>
      </div>
    </div>
</div>