<div id="modal-no-backdrop-blur" tabindex="-1" class="modal no-blur" data-close-modal>
  <div class="modal-content">
    <div class="header">
      <h3 class="h3 mt-0">Without Blur</h3>
      <button type="button" class="btn-close" data-close-modal aria-label="Close"></button>
    </div>
    <div class="body">Overlay should be displayed without blur.</div>
    <div class="footer text-right">
      <button class="btn btn-primary" data-open-modal="documentation-modal" data-documentation="components/modal">Documentation</button>
      <button class="btn btn-neutral" data-close-modal>Close</button>
    </div>
  </div>
</div>